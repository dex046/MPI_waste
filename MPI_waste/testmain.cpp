...
int main(int argc, char ** argv)
{
    ...
    int cpu_x = 4, cpu_z = 5;
    Partition pt(*Pa, nnx, nnz, 4, 4, cpu_x, cpu_z, rank, p_size);//borderlength = 4
    ...
    int inside_length_x = pt.getinsideLength_x();
    int inside_length_z = pt.getinsideLength_z();

    ip->GradVp = new float[inside_length_x * inside_length_z];
    memset((void *)ip->GradVp, 0, sizeof(float) * inside_length_x * inside_length_z);
    ip->ObjIter = new float[ip->IterN];
    memset((void *)ip->ObjIter, 0, sizeof(float) * ip->IterN);

    // 炮信息
    int is_num = pt.getShot_num();
    ...
    {
        ip->St[i].rn = 510;
        ip->St[i].s.Sx = Pa->PMLx + is * 10 + 200;
        ip->St[i].s.Sz = Pa->PMLz + 2;
        ip->St[i].re = new RL[ip->St[i].rn];
        memset((void*)ip->St[i].re, 0, sizeof(RL) * ip->St[i].rn);

        // 检波器位置
        for (uint m = 0; m < ip->St[i].rn; m++)
        {
            ip->St[i].re[m].Rx = Pa->PMLx + 1 * m;
            ip->St[i].re[m].Rz = Pa->PMLz + 2;
        }
    }
